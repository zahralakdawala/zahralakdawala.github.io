...
def get_modules(x):
    ...
    return list(filter(lambda x: os.path.basename(x) not in [BASE,JSON],modules))
...
def module_setup(y):
    ...
    data = {}
    for x in files:
        data['title'] = os.path.basename(x).split('.')[0].replace('_',' ')
        data['link'] = fix_path(x.replace(PATH,LINK))
        data['type'] = type_setter(x)
    return data
